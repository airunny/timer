...
type Token struct {
	...
	AccessToken      string    `gorm:"column:access_token;type:varchar(128) not null;default:'';index:,unique;comment:用户token"`
	ExpiresIn        int64     `gorm:"column:expires_in;type:bigint unsigned not null;default:0;comment:token过期时间，单位秒"`
	RefreshToken     string    `gorm:"column:refresh_token;type:varchar(127) not null;default:'';index:,unique;comment:刷新token"`
	RefreshExpiresIn int64     `gorm:"column:refresh_expires_in;type:varchar(127) not null;default:0;comment:刷新token过期时间，单位秒"`
